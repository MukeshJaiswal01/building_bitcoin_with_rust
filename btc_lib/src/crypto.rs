...
impl Signature {
    ...
    // sign a crate::type::TransactionOutput from its Sha256 hash
    ...
    fn load<I: Read>(mut reader: I) -> IoResult<Self> {
       ...
       //  decode the public key from PEM
        let public_key = buf.parse().map_err(|_| {
            ...
            IoError::new(IoErrorKind::InvalidData, "failed to parse public key")
...
mod signkey_serde { 
    ...
    pub fn serialize<S>(key: &super::SigningKey<super::Secp256k1>, serializer: S) 
        -> Result<S::Ok, S::Error>
        where 
            S: serde::Serializer
        {
    ...
    pub fn deserialize<'de, D> (
        ...
        where D: serde::Deserializer<'de> 
        {
           ...
           // create a signing key from a raw slice of bytes.
